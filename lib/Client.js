...
function Client(options) {
  ...
  this.getRepresentation = function() {
    ...
    var cssFiles = template.css ? template.css : [];
    cssFiles.unshift(process.cwd() + '/node_modules/mocha/mocha.css');

    var scriptFiles = template.scripts ? template.scripts : [];
    ...
    for(var i = 0 ; i < testsFiles.length ; i++) {
      templateContext.tests.push({ path: fileReferencePrefix + testsFiles[i].path });
      referencedFiles.push(testsFiles[i].path);
    }

    for(var i = 0 ; i < scriptFiles.length ; i++) {
      templateContext.scripts.push({ path: fileReferencePrefix + scriptFiles[i] });
      referencedFiles.push(scriptFiles[i]);
    }

    for(var i = 0 ; i < cssFiles.length ; i++) {
      templateContext.css.push({ path: fileReferencePrefix + cssFiles[i] });
      referencedFiles.push(cssFiles[i]);
    }
