...
function Server(config) {
  ...
  this.start = function(serverStartedCallback) {
    ...
    childProcess = child_processes.spawn('node', options, {cwd: process.cwd(), env: process.env});

    attachToLogs(childProcess, options.log);
  ...
  function attachToLogs(process, logOptions) {
